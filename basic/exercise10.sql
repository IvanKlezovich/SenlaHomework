...
insert into cd.members (memid, surname, firstname, address,
                        zipcode, telephone, recommendedby, joindate) values
...
/**
  Так делать нельзя т.к. поля полностью не совпадают, поэтому это не совсем правильно.
  Но т.к. поле первого запроса больше, поэтому все прошло нормально.
 */
